...
> [!NOTE]
> - **AI Model**: Toàn bộ quá trình tự động phân loại tag cho Menu và Restaurant được thực hiện bằng model **Gemini 3.1 Flash Lite**.
> - Hiện tại tất cả các tag `cuisine_type` đều trả về kết quả là `vietnamese`, vấn đề này sẽ được tìm cách khắc phục trong tương lai.
...
| `taste_tags` | `jsonb` | ✓ | — | | Mảng tổng hợp các vị từ menu (không trùng) | `["savory", "spicy"]` |
| `context_tags` | `jsonb` | ✓ | — | | Mảng tổng hợp ngữ cảnh phù hợp | `["casual", "study"]` |
| `style_tags` | `jsonb` | ✓ | — | | Mảng tổng hợp phong cách | `["street_food", "cafe"]` |
| `environment_tags` | `jsonb` | ✓ | — | | Mảng tổng hợp không gian quán | `["indoor", "outdoor"]` |
| `cuisine_type` | `jsonb` | ✓ | — | | Mảng tổng hợp loại hình ẩm thực | `["vietnamese"]` |
| `embedding_vector` | `jsonb` | ✓ | — | | Vector nhúng phục vụ gợi ý AI | `[0.12, -0.34, ...]` |

> [!IMPORTANT]
> **Indexing Requirement:** Hệ thống Recommendation sẽ thường xuyên thực hiện các truy vấn lọc dữ liệu theo tags. Do đó, cần tạo **GIN Index** cho các cột `taste_tags`, `context_tags`, `style_tags`, `environment_tags`, và `cuisine_type` trong bảng `restaurant`. Điều này giúp hệ thống truy cập trực tiếp vào dữ liệu qua "mục lục" (ví dụ tìm món cay thay vì quét Full Table Scan 50000+ nhà hàng), từ đó tối ưu hóa tốc độ truy vấn đáng kể cho Recommendation Engine, Homepage Feed, Favorites, Search, Chatbot, và Top-K ranking.
...
| `taste_tags` | `jsonb` | ✓ | — | | Mảng vị của món | `["savory", "spicy"]` |
| `style_tags` | `jsonb` | ✓ | — | | Phong cách phục vụ | `["fast_food"]` |
| `cuisine_type` | `jsonb` | ✓ | — | | Xuất xứ/thể loại ẩm thực | `["vietnamese"]` |

**Các giá trị hợp lệ:**

| Cột | Các giá trị có thể |
| :--- | :--- |
| `taste_tags` | `"spicy"`, `"sweet"`, `"savory"`, `"sour"`, `"bitter"`, `"umami"`, `"creamy"`, `"fresh"`, `"light"`, `"smoky"`, `"crispy"` |
| `style_tags` | `"street_food"`, `"fine_dining"`, `"cafe"`, `"rooftop"`, `"buffet"`, `"fast_food"`, `"restaurant"`, `"snack"` |
| `cuisine_type` | `"vietnamese"`, `"japanese"`, `"korean"`, `"thai"`, `"chinese"`, `"western"`, `"indian"`, `"italian"` |
...
| `favorite_type` | `varchar` | ✗ | — | | Loại yêu thích (`restaurant` hoặc `menu`) | `"restaurant"` |
...
> [!NOTE]
> Một record `favorites` có thể lưu yêu thích theo **món ăn cụ thể** (`menu_id`) hoặc theo **nhà hàng** (`restaurant_id`). Cột `favorite_type` được sử dụng để phân định rõ ràng nhằm hỗ trợ xử lý phía backend và tránh dữ liệu mơ hồ.
...
| `score` | `real` | ✓ | — | | Điểm số tương đồng/gợi ý (0.0–10.0) | `8.6` |
